...
export async function AddStudent(
  ...
  try {
    ...
    if (existingUser) {
      // If this is an inactive placeholder (created during checkout email-verify),
      // update it instead of creating a duplicate
      if (existingUser.status_id === 2 && !existingUser.is_paid) {
        await prisma.user.update({
          where: { email },
          data: { fname, lname, user_type_id, status_id, student_type },
        });
      } else {
        return {
          status: "error",
    ...
    // Create the user (only if they didn't already exist as a placeholder above)
